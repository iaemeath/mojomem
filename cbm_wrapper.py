...
class CBMWrapper:
    ...
    def _spawn(self):
        ...
        try:
            ...
            self.process = None
            print(f"⚠️ Failed to spawn codebase-memory-mcp: {e}")
            return
        ...
        if "error" in init_resp:
            ...
            try:
                ...
                pass
            print("✅ codebase-memory-mcp core spawned and initialized.")
    ...
    def send_request(self, method: str, params: dict = None) -> dict:
        ...
        if not self.process or self.process.poll() is not None:
            print("⚠️ CBM core exited, respawning...")
            self._spawn()
            if not self.process:
        ...
        if "error" in resp and (not self.process or self.process.poll() is not None):
            print("⚠️ CBM core died during request, respawning and retrying...")
            self._spawn()
            if self.process:
                resp = self._send_raw(method, params)
